...
namespace GamePlay.Server.Model
{
	...
	public class ServerRoundStatus
	{
		...
		{
			CheckRange(index);
			int i = openMelds[index].FindIndex(meld =>
				meld.Type == MeldType.Triplet && meld.First.EqualsIgnoreColor(kong.First));
			if (i < 0)
			{
		...
		{
			discardZhenting[playerIndex] =
				MahjongLogic.TestDiscardZhenting(handTiles[playerIndex], rivers[playerIndex]);
		}
		...
		public void NextRound(int dice, bool next = true, bool extra = false, bool keepSticks = false)
		{
			...
			Debug.Log(
				$"Entering next round, total players: {GameSettings.MaxPlayer}, current player index: {oya}, dice: {dice}, extra: {this.extra}");
			kongClaimed = 0;
		...
		private void CheckRange(int index)
		{
			if (index < 0 || index >= GameSettings.MaxPlayer)
				throw new IndexOutOfRangeException(
					$"Player index out of range, should be within {0} to {GameSettings.MaxPlayer - 1}");
		}
