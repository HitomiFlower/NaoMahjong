...
namespace GamePlay.Server.Controller.GameState
{
	public class PlayerKongState : ServerState, IOnEventCallback
	{
		...
		public override void OnServerStateEnter()
		{
			...
			firstTime = Time.time;
			serverTimeOut = CurrentRoundStatus.MaxBonusTurnTime + gameSettings.BaseTurnTime +
		...
		private void NextState()
		{
			...
			if (outTurnOperations.Any(op => op.Type == OutTurnOperationType.Rong))
			{
				...
				ServerBehaviour.Instance.TurnEnd(CurrentPlayerIndex, discardingTile, false, outTurnOperations, true,
					false);
				return;
			}
