...
namespace Console
{
	public static class ConsoleCommands
	{
		...
		[ConsoleMethod("alter_yama",
			"Set the upcoming tiles in current mahjong set. If game is not started, this command has no effect")]
		public static bool SetTiles(string tileString)
		{
			var tiles = ParseTiles(tileString);
			var fieldInfo =
				typeof(ServerBehaviour).GetField("mahjongSet", BindingFlags.NonPublic | BindingFlags.Instance);
		...
		[ConsoleMethod("alter_yama_reverse",
			"Set the upcoming lingshang tiles in current mahjong set. If game is not started, this command has no effect")]
		public static bool ReplaceLingShang(string tileString)
		{
			...
			var methodInfo =
				typeof(MahjongSet).GetMethod("SetTilesReverse", BindingFlags.NonPublic | BindingFlags.Instance);
			methodInfo.Invoke(set, new[] {tiles});
			return true;
		}
