...
//
//		/// <summary>
//		/// Prevent calling the constructor of the AudioManager
//		/// </summary>
//		private AudioManager() { }
		...
		void Start()
		{
			if (musicSource != null)
			{
				...
				// spent almost 4 hours trying to figure out why... well didn't know until I ran some tests 
				Timing.RunCoroutine(OnUpdate(), Segment.Update);
			}
		}
		...
		/// <summary>
		/// Plays a background music by a BgmId
		/// </summary>
		/// <param name="bgmId">BGB ID</param>
		/// <param name="musicTransition">How should the music change from the current to the next</param>
		/// <param name="duration">Time in secs it takes to transition.</param>
		/// <param name="volume">Playback volume</param>
		public void PlayBgm(BgmId bgmId, MusicTransition musicTransition = MusicTransition.CrossFade, float duration = 1f, float volume = 1f)
		{
			...
			var clipName = BgmPath + GetClipNameForBgm(bgmId);
			var clip = GetClipFromPlaylist(clipName);
			if (clip == null)
			{
		...
		{
			// Stop if trying to play the same music
		...
		/// <summary>
		/// Play a sound effect for one shot
		/// </summary>
		/// <param name="seId">SE ID</param>
		/// <param name="compCallback">Action callback to be invoked after the sound has finished.</param>
		public void PlaySe(SeId seId,  Action compCallback = null)
		{
			if (seId == SeId.None)
			{
				compCallback?.Invoke();
			...
			var clip = Resources.Load<AudioClip>(SePath + GetClipNameForSe(seId));
			
			PlaySFX(clip, clip.length, false, compCallback);
		}

		/// <summary>
		/// Play a sound effect for loop
		/// </summary>
		/// <param name="seId">SE ID</param>
		/// <param name="duration">The length in time the clip should play</param>
		/// <param name="compCallback">Action callback to be invoked after the sound has finished.</param>
		public void PlayLoopSe(SeId seId, float duration, Action compCallback = null)
		{
			if (seId == SeId.None)
			{
				compCallback?.Invoke();
			...
			PlaySFX(clip, duration, false, compCallback);
		}
